 ...
 * 
 */
import '../assets/stylesheets/css/app-tablet-hontam.css';
...
class AppTablet extends Component{
	...
	componentWillMount() {
		...
		var register = cookie.load('register');
		var opt = {url: API.info(), data: {key:register.key, langid: setting.langid}};
		var smile = [];
		var rating = [];
		var background = [];
		eCommon.request(opt, function(response){ // get information
			eCommon.logs(response);
			if(response.length > 0){
				smile = response[0].arrsmile;
				rating = response[0].arrrating;
				background = response[0].arrbackground;
			}
		});
	}
	
	componentDidMount(){
		var langid = cookie.load("langid");
	...
	sendOther(rid, opinion){
		...
		var opt = {
			url: API.comment(),
			type: 'POST',
			data: {comment: opinion, id: rid, key: register.key}
		}
	...
	setRID(ridReceived){
		...
		var opt = {
			url: API.smile(),
			type: 'POST',
			data: {id: ridReceived, key: register.key}
		}
		eCommon.request(opt, function(response){
			     ...
			     <div className="views">
				    <div className="view view-main">
				      <div className="pages">
					      ...
					      <Page_1 setRID={this.setRID.bind(this)} 
				      	  		sendOther={this.sendOther.bind(this)} 
				      	  		title={this.state.title} button={this.state.btn_popup} 
				      	  		text={this.state.text_p1} ip={this.state.ip} 
				      	  		logo={this.state.logo} pages={this.state.pages1} 
...
class Page_1 extends React.Component {
   ...
   itemClick(item, event){
	   var _=this;
	   var $this = $(ReactDOM.findDOMNode(this.refs["rating__" + item.id]));
	   ...
	   if(Number(item.next) == -1){
		   app.modal({
			    ...
			    buttons: [
			      {
			        text: _.props.button[1],
			      },
			      {
			        text:  _.props.button[0],
			        onClick: function() {
			           if($('#opinion_text').val() != ""){
			        	   _.props.sendOther(item.id, $('#opinion_text').val());
			        	   // remove ripple
		             	...
		             	{this.props.pages.map(function(item, index){
		             		return (
		             			<div key={item.id + '_' + item.langid} className="col-15">
		             				<a href={'#esmile_page_' + item.next } ref={'rating__' + item.id} className="ripplelink" onClick={this.itemClick.bind(this, item)}>
...
class Button extends React.Component {
	...
	send(){
		   console.log('>> send rating ' + this.props.rid);
		   var $this = $(ReactDOM.findDOMNode(this.refs.btn_send));
		   var rating = [];
		   var $span = $this.parent().parent().parent().find('span.check_round');
		   var length = $span.length;
		   for(var i = 0; i < length; i++){
			   rating.push($span.eq(i).attr('data-id'));
			   console.log($span.eq(i).attr('data-id'));
		   }
		  $this.parent().parent().parent().find('span').remove();
		  var register = cookie.load('register');
			var opt = {
				url: API.rating(),
				type: 'POST',
				data: {id: this.props.rid, rating:rating, key: register.key}
			}
			eCommon.request(opt, function(response){
				eCommon.logs(response);
			});
